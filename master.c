...
int master(char * datosIni, int ntareas, int JobID, double Start) {
    ...
    printf("INFO: Master.master -> Elitism:%d\n",elitism);
    ...
    while(numgen < numGenerations) {
        ...
        printf("INFO: Master.master -> Before genetic\n");
        ...
        if (!(RecyclePopulations)) {
            if(GENETIC_Init_Farsite(elitism,pCrossover,pMutation,range_file,bests_indv,1,0,crossMethod,nFuels)<1) {
                printf("ERROR: Master.master -> Error when initializing Genetic Algorithm! Exiting...\n");
                return -1;
            }
            if(GENETIC_Algorithm_Farsite(&p, new_pob_str,nFuels,pending)<1) {
                printf("ERROR: Master.master -> Error when Running Genetic Algorithm! Exiting...\n");
                return -1;
            }
        }
    ...
    sprintf(gen_str, "%d", numgen);
    pob_str_errors = str_replace(population_error, "$1", gen_str);
    printf("INFO: Master.master -> Población predicción final\n");
    print_population_farsite(p);
    ...
    if (( ErrorFile = fopen(ErrorFileName, "w")) == NULL) {
        printf("ERROR: Master.master -> Error when opening file 'w' %s\n", ErrorFileName);
    } else {
...
int initMaster(char * filename, int nworkers) {
    ...
    if ( (ClassF=fopen(ClassesFile,"r")) == NULL ) {
        printf("ERROR: Master.initMaster -> Error loading Classes file %s.\n",ClassesFile);
        exit(0);
    } else {
        fscanf(ClassF,"NumClasses:%d\n",&nClasses);
        if ((nClasses < 0 || nClasses > 100)) {
            printf("ERROR: Master.initMaster -> Invalid classes file.\n");
            exit(0);
        }
        ...
        int i;
        printf("INFO: Master.initMaster -> Loaded Classes:\n");
        for (i=0; i<nClasses; i++) {
            fscanf(ClassF, "%c:%d\n", &(ClassesLabel[i]), &(ClassesCores[i]));
            printf("INFO: Master.initMaster -> %c:%d\n", ClassesLabel[i],ClassesCores[i]);
    ...
    nFuels=0;
    printf("INFO: Master.initMaster -> Calibrate Adjustments:%d \n", CalibrateAdjustments);
    if (CalibrateAdjustments) {
        ...
        printf("ERROR: Master.initMaster -> FUELS:%d to calibrate.\n",nFuels);
...
void fin_workers(int nworkers) {
    printf("INFO: Master.fin_workers -> STOP signal to workers.\n");
...
int SearchClassIndex(char let) {
    ...
    printf("WARNING: Master.repartirPoblacionFarsite_Classes -> [W]WARNING:----->Error Class not found!<----\n");
    return 1;
}
...
int repartirPoblacionFarsite_Classes(POPULATIONTYPE *p, int nworkers) {
    ...
    poblacion = p->popu_fs;
    c=nClasses-1;
    printf("INFO: Master.repartirPoblacionFarsite_Classes -> Classe mas pesada: %c\n", ClassesLabel[c] );
    c=nClasses-1;
    while ((c>=0) && go) {
    ...
    while ( ((block_received < (numind-(elitism+staticPending))) && (numgen!=0)) || ((block_received < numind) && (numgen==0) && para ) ) {
        while ( (((block_sended < numind) && numgen==0 ) && goon) ||  (((block_sended < (numind-(elitism+staticPending))) && numgen!=0 ) && goon)  ) {
            if (c >= 0) {
                ...
                if (poblacion[id_count].executed == 0) {
                    ...
                    printf("INFO: Master.repartirPoblacionFarsite_Classes -> Master tiene posicion %d para %d individuo con %d threads\n",pos,id_count,poblacion[id_count].threads);
                    if (pos != -1) {
                        id_count= (it->Ind)->id;
                        ...
                        printf("INFO: Master.repartirPoblacionFarsite_Classes -> Se envia al Worker %d que ocupa %d cores, %d.\n",pos+1,poblacion[id_count].threads,id_count);
                        block_sended++;
                        printf("INFO: Master.repartirPoblacionFarsite_Classes -> Master block_sended: %d\n", block_sended);
                        if (it->Next == NULL) {
                            go = 1;
                            c--;
                            while ((c >= 0) && go) {
                                if (Classified[SearchClassIndex(ClassesLabel[c])].Classes != NULL) {
                                    it = Classified[SearchClassIndex(ClassesLabel[c])].Classes;
                                    printf("INFO: Master.repartirPoblacionFarsite_Classes -> Pasamos a clase:%c c=%d SearchCI:%d\n",ClassesLabel[c],c,SearchClassIndex(ClassesLabel[c]));
                                    go = 0;
                                } else {
                                    c--;
                                }
                    ...
                    printf("INFO: Master.repartirPoblacionFarsite_Classes -> Master ya ejecutado, paso al siguiente\n");
                    if (it->Next == NULL) {
                        go = 1;
                        c--;
                        while ((c >= 0) && go) {
        ...
        while((received && (block_received < (block_sended + pending ))) || !goon  ) {
            ...
            if ((MPI_Wtime() - StartingTime) > 1300.0) {
                ...
                printf("INFO: Master.repartirPoblacionFarsite_Classes -> Pendientes, pending, para la siguiente generacion:%d. %d Individuos ejecutados en gen:%d\n",pending,block_received,numgen);
                goon = 1;
            } else {
                ...
                if (pos != -1) {
                    ...
                    printf("INFO: Master.repartirPoblacionFarsite_Classes -> Se libera de Worker %d %d cores ejecucion individuo %d con error %1.6f .\n",pos,p->popu_fs[individualID].threads,individualID,p->popu_fs[individualID].error);
    ...
    printf("INFO: Master.repartirPoblacionFarsite_Classes -> TERMINO MASTER\n");
}
    ...
    while (block_received < num_ind_pred) {
        while ( (block_sended < num_ind_pred) && goon) {
            if (c >= 0) {
                ...
                if (poblacion[id_count].executed == 0) {
                    pos = getAvailCore(worker_busy, nworkers,poblacion[id_count].threads);
                    printf("INFO: Master.prediccionPoblacionFarsite -> Master tiene posicion %d para %d individuo con %d threads\n",pos,id_count,poblacion[id_count].threads);
                    if (pos != -1) {
                        id_count= (it->Ind)->id;
                        //printf("Master id_count:%d\n",id_count);
                        Master_SendMPI_SetOfIndividualTask(pos + 1, chunkSize, id_count, numgen, numind, poblacion);
                        worker_busy[pos] = worker_busy[pos] + poblacion[id_count].threads;
                        printf("INFO: Master.prediccionPoblacionFarsite -> Se envia al Worker %d que ocupa %d cores, %d.\n",pos+1,poblacion[id_count].threads,id_count);
                        block_sended++;
                        if (it->Next == NULL) {
                            go = 1;
                            c--;
                            while ((c >= 0) && go) {
                                if (Classified[SearchClassIndex(ClassesLabel[c])].Classes != NULL) {
                                    it = Classified[SearchClassIndex(ClassesLabel[c])].Classes;
                                    printf("INFO: Master.prediccionPoblacionFarsite -> Pasamos a clase:%c c=%d SearchCI:%d\n",ClassesLabel[c],c,SearchClassIndex(ClassesLabel[c]));
                                    go = 0;
                                } else {
                                    c--;
                                }
                    ...
                    printf("INFO: Master.prediccionPoblacionFarsite -> Master ya ejecutado, paso al siguiente\n");
                    if (it->Next == NULL) {
                        go = 1;
                        c--;
                        while ((c >= 0) && go) {
        ...
        while(received && (block_received < block_sended)) {
            ...
            if (pos != -1) {
                printf("INFO: Master.prediccionPoblacionFarsite -> received: %d block_received :%d block_sended:%d\n",received,block_received,block_sended);
                worker_busy[pos-1] = worker_busy[pos-1] - poblacion[individualID].threads;
                poblacion[individualID].executed = 1;
                printf("INFO: Master.prediccionPoblacionFarsite -> Se libera de Worker %d %d cores ejecucion individuo %d con error %1.6f .\n",pos,p->popu_fs[individualID].threads,individualID,p->popu_fs[individualID].error);
                block_received++;
                goon = 1;
                received = 0;
            } else {
...
int apply_EVOLUTE(POPULATIONTYPE * P) {
    ...
    if (strcmp(alg, "GA") == 0) {
        printf("INFO: Master.apply_EVOLUTE -> Aplicando GENETICO\n");
        //GENETIC_Algorithm(&p);
    }
}
...
int NewClassifyPopulationFARSITE_FAKE(POPULATIONTYPE * pobla, int numgen) {
    ...
    printf("INFO: Master.NewClassifyPopulationFARSITE_FAKE -> BEGIN:Classify population\n");
    ...
    */
    ...
    printf("INFO: Master.NewClassifyPopulationFARSITE_FAKE -> END:Classify FAKE population\n");
    //free(tempName);
...
int NewClassifyPopulationFARSITE(POPULATIONTYPE * pobla, int numgen) {
    ...
    printf("INFO: Master.NewClassifyPopulationFARSITE -> BEGIN:Classify population\n");
    /*Establecer la ruta de las clases de Weka*/
    printf("INFO: Master.NewClassifyPopulationFARSITE -> export CLASSPATH=\"%s:%s:$CLASSPATH\"\n",WekaPath,ClassifyPATH);
    sprintf(nt,"");
    sprintf(nt,"export CLASSPATH=\"%s:%s:$CLASSPATH\"",WekaPath,ClassifyPATH);
    putenv(nt);
    sprintf(tempName,"");
    ...
    if ( !((temp = fopen(tempName, "w+")) == NULL  )) {
        ...
        printf("ERROR: Master.NewClassifyPopulationFARSITE -> Error opening file %s\n",tempName);
    }
    ...
    printf("INFO: Master.NewClassifyPopulationFARSITE -> END:Classify population\n");
    free(tempName);
    //if (numgen==0){exit(0);}
}
...
int ClassifyPopulationFARSITE_WORKS(POPULATIONTYPE * pobla, int numgen) {
    ...
    FILE * cagonto;
    char * tempName = (char*)malloc(sizeof(char) * 500);
    printf("INFO: Master.ClassifyPopulationFARSITE_WORKS -> BEGIN:Classify population\n");
    /*Establecer la ruta de las clases de Weka*/
    printf("INFO: Master.ClassifyPopulationFARSITE_WORKS -> export CLASSPATH=\"%s:$CLASSPATH\"\n",WekaPath);
    sprintf(nt,"");
    ...
    if ( !((temp = fopen(tempName, "w+")) == NULL  )) {
        ...
        printf("ERROR: Master.ClassifyPopulationFARSITE_WORKS -> Error opening file %s\n",tempName);
    }
    /*Llamada al sistema para utilizar Weka*/
    ...
    if ( !((cagonto = fopen(tempName, "r")) == NULL) ) {
        ...
        printf("INFO: Master.ClassifyPopulationFARSITE_WORKS -> Error opening file %s\n",tempName);
    }

    printf("INFO: Master.ClassifyPopulationFARSITE_WORKS -> END:Classify population\n");
    // free(tempName);
    //if (numgen==0){exit(0);}
}
...
int SearchCoreClass(char let) {
    ...
    printf("INFO: Master.SearchCoreClass -> [W]WARNING:----->Error Class not found!<----\n");
    return 1;
}
